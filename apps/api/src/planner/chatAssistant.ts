...
const buildChatPrompt = (message: string, history: ChatMessage[]): string => {
  ...
  return [
    ...
    '- Even if enough details are known, stay in DISCOVERY until the user explicitly confirms they are done and asks to build/generate the workflow.',
    '- Only switch to PROPOSAL after explicit user confirmation such as "that\'s all", "no more", "build the workflow", or "generate the plan now".',
